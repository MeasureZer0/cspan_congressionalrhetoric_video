# OpenCV for handling video files (reading frames, opening videos)
import cv2
# Pandas for reading and handling CSV files in table format
import pandas as pd
# OS for safe file and directory path operations
import os
# NumPy for storing frames as numpy arrays
import numpy as np
...
def extract_frames(cap, frame_skip: int = 5) -> list:
    """
    ...
    # - cv2.destroyAllWindows(): closes any OpenCV windows (uncomment during visualization)
    cap.release()
...
def load_data(data_dir: str, label_file: str):
    """
